-- Allow one-sided (real + TBD) results; TBD side uses NULL player id and 0 in lo/hi.
-- Mark group standings winners on tournament_group_player.
...
CREATE TABLE tournament_result_new (
    ...
    tournament_player_a_id INTEGER REFERENCES tournament_player (id) ON DELETE CASCADE,
    tournament_player_b_id INTEGER REFERENCES tournament_player (id) ON DELETE CASCADE,
    ...
    CHECK (tournament_player_a_id IS NOT NULL OR tournament_player_b_id IS NOT NULL)
);
...
ALTER TABLE tournament_group_player ADD COLUMN is_winner INTEGER NOT NULL DEFAULT 0 CHECK (is_winner IN (0, 1));
